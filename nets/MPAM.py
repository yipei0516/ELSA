...
class SemanticFusionModule(nn.Module):
    ...
    def forward(self, v_stage, a_stage):
        v_interact = self.vadaptiveinteraction(v_stage, v_stage, v_stage)  # [nbatch*10, stage_num, 512] : unit-level transformer interaction
        a_interact = self.aadaptiveinteraction(a_stage, a_stage, a_stage)  # [nbatch*10, stage_num, 512]
        v_weight = torch.sigmoid(self.vselectfusion(v_interact)) # [nbatch*10, stage_num, 1] : 學習融合權重（selective attention）代表每層對於該 segment 的重要性
...
class SemanticCaptureModule(nn.Module):
    def __init__(self, num_inputs, ffn_dim, num_channels, kernel_size=3, dropout=0.2, nhead=8):
        ...
        for i in range(num_levels):
            ...
            msa_layers += [IntegrateAttentionBlock(TransformerLayer(num_inputs,
                                                                    ...
                                                                    c(self.feedforward), dropout), # 對 a 做 self-attention
                                                   TransformerLayer(num_inputs,
                                                                    MultiHeadAttention(nhead, num_inputs,
                                                                                       masksize=dilation_size * 2),
                                                                    c(self.feedforward), dropout), # 對 v 做 self-attention
                                                   TransformerLayer(num_inputs,
                                                                    MultiHeadAttention(nhead, num_inputs,
                                                                                       masksize=dilation_size * 2),
                                                                    c(self.feedforward), dropout), # cross-attention
                                                    num_inputs
                                                   )]
        self.vnetwork = nn.Sequential(*v_layers)
        self.anetwork = nn.Sequential(*a_layers)
        self.msa = nn.Sequential(*msa_layers) # multimodal self-attention
    ...
    def forward(self, v, a):
        ...
        for i in range(len(self.vnetwork)):
            v_msa, a_msa = self.msa[i](v_stage, a_stage) # 先做attention
            v_msa = v_msa.permute(0, 2, 1).contiguous()  # [nbatch, 512, 10]
            a_msa = a_msa.permute(0, 2, 1).contiguous()  # [nbatch, 512, 10]
            v_tcn = self.vnetwork[i](v_msa) # 再通過 temporal 卷積（TemporalBlock(conv+relu+conv)），目的:產生具備位置資訊與時間上下文的特徵
        ...
        v_stage = torch.stack(v_stage_list, dim=2)  # [nbatch, 10, stage_num, 512] : 整合所有 stage 的特徵
